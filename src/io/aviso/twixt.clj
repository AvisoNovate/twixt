  ...
  (fn resolver [path]
    (let [resource-path (str root path)]
      (if-let [url (io/resource resource-path)]
        (make-asset-map content-types resource-path url)))))
...
(defn default-development-pipeline
...
(defn wrap-with-twixt-setup
  ...
  [handler twixt-options pipeline]
  (fn twixt-setup [request]
    (handler (assoc request :twixt {:pipeline pipeline
    ...
    (when (match? path path-prefix)
      (tracker/trace
        ...
        (let [resource-path (.substring path (.length path-prefix))
              pipeline (-> request :twixt :pipeline)]
          (if-let [asset-map (pipeline resource-path)]
            (-> asset-map
                :content
                io/input-stream
                r/response
