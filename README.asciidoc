...
At its core, Twixt is a Ring middleware that maps certain URL patterns to matching files on the classpath,
...
Embedded in in the middle is the checksum for the file.
...
Twixt sets headers to indicate a far-future expires header;
the upshot of which is that, once the resource for an asset is downloaded to the client, it will not ask for it again.
...
* Leverage core.async ?
* Break out the the Les•s, Jade, CoffeeScript, and exception reporting support into a-la-carte modules
