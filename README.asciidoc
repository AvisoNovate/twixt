...
;;; Create twixt wrappers and handler to handle /asset/ URLs, in production mode
(def app-handler
  (te/default-twixt-handler app)
...
File system caching is enabled normally only enabled in development mode.
